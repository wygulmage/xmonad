...
-- May make more sense to shrink the API and just write @modify $ l %~ f@.
{-# INLINE (%=) #-}

(.=) :: (State.MonadState s m)=> ASetter s s a b -> b -> m ()
l .= x = State.modify $ l .~ x
-- May make more sense to shrink the API and just write @modify $ l .~ x@.
{-# INLINE (.=) #-}
...
-}
l <~ mx = (l .=) =<< mx
-- l <~ mx = State.modify $ (l .~) =<< mx
-- l <~ mx = do
--    x <- mx
--    l .= x
-- This is probably more warranted than (.=), since writing the whole thing out is pretty clunky.
{-# INLINE (<~) #-}
